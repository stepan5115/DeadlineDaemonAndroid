...
object Parser {
    ...
    fun fromJsonToUser(jsonString: String) : User {
        ...
        val groups : String = json.getJSONArray("groups").let { array ->
        ...
        val notificationExcludedSubjects : String = json.getJSONArray("notificationExcludedSubjects").let { array ->
            (0 until array.length()).joinToString { array.getString(it) }
        }
        val completedAssignments : String = json.getJSONArray("completedAssignments").let { array ->
            (0 until array.length()).joinToString { array.getString(it) }
        }
